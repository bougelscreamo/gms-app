...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    const data = computed(() => {
      ...
      if (keyword.value != "") {
        rawData = originalData.filter((item) => {
          return item.username.toUpperCase().includes(keyword.value.toUpperCase()) || item.access.toUpperCase().includes(keyword.value.toUpperCase()) || item.action.toUpperCase().includes(keyword.value.toUpperCase()) || item.description.toUpperCase().includes(keyword.value.toUpperCase()) || item.ipaddress.toUpperCase().includes(keyword.value.toUpperCase());
        });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "content" }, _attrs))}><div class="container-fluid" style="${ssrRenderStyle({ "padding-bottom": "10px" })}"><div class="card card-primary"><div class="card-header"> Audit Trails </div><div class="card-body"><div class="row"><div class="col-md-12"><div role="group" class="input-group mb-3 input-group-md"><div class="input-group-prepend"><div class="input-group-text">Search...</div></div><input type="text" class="form-control"${ssrRenderAttr("value", unref(keyword))}><div class="input-group-append"><button text="Button" type="button" class="btn btn-secondary btn-md"${ssrIncludeBooleanAttr(!unref(keyword)) ? " disabled" : ""}>X</button></div></div></div></div></div><div class="card-body bg-dark table-responsive" id="theContainer"><table class="table table-bordered" width="100"><thead class="bg-secondary"><tr><th>Date Time</th><th>User Name</th><th>Access</th><th>Action</th><th>Description</th><th>IP Address</th></tr></thead><tbody><!--[-->`);
      ssrRenderList(unref(data), (audit, index) => {
        _push(`<tr><td>${ssrInterpolate(audit.datetimes)}</td><td>${ssrInterpolate(audit.username)}</td><td>${ssrInterpolate(audit.access)}</td><td>${ssrInterpolate(audit.action)}</td><td>${ssrInterpolate(audit.description)}</td><td>${ssrInterpolate(audit.ipaddress)}</td></tr>`);
      });
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/AuditTrails/index.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=index-2c96b1b9.mjs.map
