...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    let totalRow = ref(0);
    let totalPage = ref(0);
    totalRow = store.$state.data.users.data.length;
    ...
    const data = computed(() => {
      ...
      if (keyword.value != "") {
        rawData = originalData.filter((item) => {
          return item.fullname.toUpperCase().includes(keyword.value.toUpperCase()) || item.username.toUpperCase().includes(keyword.value.toUpperCase());
        });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "content" }, _attrs))}><div class="container-fluid" style="${ssrRenderStyle({ "padding-bottom": "10px" })}"><div class="card card-primary"><div class="card-header">Users</div><div class="card-body"><div class="row"><div class="col-md-12"><div role="group" class="input-group mb-3 input-group-md"><div class="input-group-prepend"><div class="input-group-text">Search...</div></div><input type="text" class="form-control"${ssrRenderAttr("value", unref(keyword))}><div class="input-group-append"><button text="Button" type="button" class="btn btn-secondary btn-md"${ssrIncludeBooleanAttr(!unref(keyword)) ? " disabled" : ""}> X </button></div></div></div></div></div><div class="card-body bg-dark" id="theContainer"><div class="row"><div class="col-md-12"><div role="group" class="input-group mb-3 input-group-md"><button type="submit" class="btn btn-info"> Create New </button></div></div></div><table class="table table-bordered"><thead class="bg-secondary"><tr><th>Action</th><th>Full Name</th><th>User Name</th><th>Role Name</th><th>Status</th><th>Last Online</th></tr></thead><tbody><!--[-->`);
      ssrRenderList(unref(data), (user, index) => {
        _push(`<tr><td>`);
        ...
        _push(` | <button type="button" class="btn btn-danger"> Delete </button></td><td>${ssrInterpolate(user.fullname)}</td><td>${ssrInterpolate(user.username)}</td><td>${ssrInterpolate(user.rolename)}</td><td>`);
        if (user.status == 1) {
...
//# sourceMappingURL=index-8f6fcaac.mjs.map
